# Tele-Shell - Privacy & Telemetry Policy

## Overview

Tele-Shell uses **PostHog** for privacy-first analytics to understand how the application is being used. This helps us improve the user experience.
...
- A **random anonymous UUID** is generated on first run and stored locally in `.teleshell_telemetry_id`
...
When you first start Tele-Shell, telemetry is **enabled by default**. You have full control:
...
Tele-Shell is **open source**. You can:
- Review the telemetry code in `teleshell.py` (TelemetryManager class)

...
MEMORY_FILE = ".teleshell_memory.json"
CHROMA_DB_PATH = ".teleshell_chromadb"
CHROMA_COLLECTION_NAME = "teleshell_memory"
OFFLINE_MODEL_PATH = ".teleshell_offline_model"
OFFLINE_MODEL_NAME = "microsoft/phi-2"
BACKUP_DIR = ".teleshell_backups"
MAX_BACKUPS = 20
    ...
    def __init__(self, fallback_to_json=True):
        ...
        try:
            ...
            self.collection = self.chroma_client.get_or_create_collection(
                ...
                metadata={"description": "TeleShell conversation memory"}
            )
...
"""
        ...
        return f'''You are TELE, a COMPLETELY FREE artificial intelligence assistant.
...
START!'''
    ...
    def _format_history(self, history):
        ...
        formatted = []
        for msg in history:
            role = "User" if msg['role'] == 'user' else "TELE"
        ...
        try:
            ...
            if json_start >= 0 and json_end > json_start:
                ...
                if needs_final_response:
                    ...
                    print(f"\n{Fore.GREEN}TELE: {final_response}{Style.RESET_ALL}")
                    response = final_response
                else:
                    response = ai_plan.get('response', 'Operation completed!')
                    print(f"\n{Fore.GREEN}TELE: {response}{Style.RESET_ALL}")
                if self._p2p_sharing and self._p2p_sharing.is_connected and self._p2p_sharing.is_host:
                    self._p2p_sharing.broadcast_output(f"TELE: {response[:400]}")
                if results:
                ...
                print(f"\n{Fore.CYAN}TELE: {ai_text}{Style.RESET_ALL}")
                self.memory.add_conversation("assistant", ai_text)
                if self._p2p_sharing and self._p2p_sharing.is_connected and self._p2p_sharing.is_host:
                    self._p2p_sharing.broadcast_output(f"TELE: {ai_text[:400]}")
                return {"success": True, "message": ai_text}
        except json.JSONDecodeError:
            print(f"\n{Fore.YELLOW}TELE: {ai_text[:500]}{Style.RESET_ALL}")
            return {"success": True, "message": ai_text}
        except Exception as e:
            ...
            try:
                ...
                if local_vars.get('condition_met') is True:
                    ...
                    if auto_fix:
                        ...
                        with open('.tele_pending_task.txt', 'w', encoding='utf-8') as f:
                        ...
                        print(f"{Fore.YELLOW}Type '1' to Ignore, '2' to Send to TeleShell.{Style.RESET_ALL}")
                        print(f"{Fore.GREEN}You >>> {Style.RESET_ALL}", end="", flush=True)
...
class TeleShell:
    ...
    def _start_hotkey_listener(self):
        ...
        def on_hotkey():
            ...
            try:
                import pyautogui
                temp_path = os.path.join(os.environ.get('TEMP', '.'), 'tele_context.png')
    ...
    def show_banner(self):
        ...
        print(f"""
{Fore.CYAN}========================================================
          TELE v9.2.0 - Advanced AI Shell + SENTINEL 1.5
        ...
        try:
            ...
            while True:
                try:
                    ...
                    if not user_input:
                        if os.path.exists('.tele_pending_task.txt'):
                            try:
                                with open('.tele_pending_task.txt', 'r', encoding='utf-8', errors='replace') as f:
                                    pending_text = f.read().strip()
                            finally:
                                try:
                                    os.remove('.tele_pending_task.txt')
                                except:
                                    pass
                    ...
                    if user_input.strip() == '2' and self.watch_manager.triggered_watches:
                        ...
                        print(f"\n{Fore.MAGENTA}Sending watch condition to TeleShell: {triggered_condition}{Style.RESET_ALL}")
                        user_input = f"A background monitoring system just detected that this condition is currently TRUE: '{triggered_condition}'. Please investigate the system to find out why this happened and provide commands to resolve or fix the underlying issue."
                        self.brain.switch_mode('fixer', permanent=False)
                        ...
                        if 'on' in user_input.lower():
                            if not PYAUTOGUI_AVAILABLE or keyboard is None:
                                ...
                                if choice == 'Y':
                                    ...
                                    print(f"\n{Fore.GREEN}Installed! Please restart Tele-Shell to use GUI.{Style.RESET_ALL}")
                                continue
                        ...
                        if 'on' in user_input.lower():
                            if not DDGS_AVAILABLE and not (REQUESTS_AVAILABLE and BS4_AVAILABLE):
                                ...
                                if choice == 'Y':
                                    ...
                                    print(f"\n{Fore.GREEN}Installed! Please restart Tele-Shell to use research.{Style.RESET_ALL}")
                                continue
                    ...
                    if user_input.lower().startswith('memory'):
                        if 'clear' in user_input.lower():
                            ...
                            for msg in history:
                                role = "You" if msg['role'] == 'user' else "TELE"
                    ...
                    if intents['needs_image_analysis'] and intents['image_path']:
                        ...
                        if analysis.get('success'):
                            print(f"\n{Fore.GREEN}TELE: {analysis.get('analysis', 'No analysis available')}{Style.RESET_ALL}")

"""
SENTINEL 1.5 - Tele-Shell Self-Preservation System

Sentinel exists to prevent Tele-Shell from making a broken system worse.

This is not a feature. This is Tele-Shell's sense of self-preservation.
...
class Sentinel:
    ...
    def evaluate_action(
        ...
        if self._is_escalating_behavior(risk_breakdown.total_score, retry_count):
            ...
            return SentinelVerdict(
                ...
                risk_breakdown=risk_breakdown,
                recommendation="TELE is taking increasingly risky actions. CONsider manual intervention.",
                sentinel_recommends_stop=True,
                lessons_applied=relevant_lessons
            )
    ...
    def _is_escalating_behavior(self, current_risk: int, retry_count: int) -> bool:
        """Detect if TELE is escalating to more dangerous actions"""
        if len(self.state.risk_escalation_trend) < 3:
            return False
    ...
    def _is_repair_loop(self, user_request: str, retry_count: int) -> bool:
        """Detect if TELE is stuck in a repair loop"""
    ...
    def generate_report(self) -> str:
        ...
        try:
            ...
            filename = f".teleshell_report_{timestamp}.md"
